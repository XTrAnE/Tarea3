...
void distanciaEntregas(Map* save)
{
  ...
  if(aux != NULL)
  {
    ...
    printf("\nLa distancia entre la entrega %i y %i es aproximadamente %i[metros]\n", sameID, numID, distancia);
  }
  else
...
void mejores3(Map* save)
{
  ...
  if(aux != NULL)
  {
    ...
    for(i = 0; i < 3; i++)
      printf("ID = %i, DISTANCIA = %i[metros]\n", datos[i].id, datos[i].distancia);
  }
  else
...
void crearRUTA(Map* save, Map* ways, List* connect)
{
  ...
  if(aux != NULL)
  {
    ...
    */
    ...
    way->total = suma;
    printf("\nDISTANCIA TOTAL RECORRIDA: %i[metros]", way->total);

    char name[100];
    printf("\n\nIngrese nombre de la nueva ruta.\n\n NOMBRE = ");
    ...
    printf("La ruta %s se guardó exitosamente.\n\n", way->nombre);
  }
  else
...
void generarRUTA(Map* save, Map* ways, List* connect)
{
  ...
  if(aux != NULL)
  {
    ...
    printf("\nLa ruta %s fue creada exitosamente y almacenada\n\n", way->nombre);
  }
  else
